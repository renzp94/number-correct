...
test('1 * 2 * 6+3. error: 6+3.不是一个数字', () => {
  expect(() => times(1, 2, '6+3.')).toThrow('6+3.不是一个数字')
})

test('2.222222222e-1 * 5.555555555e-1 * 4.444444444e-1 = 0.05486968448285322359561042524', () => {
  expect(times(2.222222222e-1, 5.555555555e-1, 4.444444444e-1)).toBe(
    '0.05486968448285322359561042524',
  )
})

test('1.23e2 * 2.45e2 * 1.23e2 = 3706605', () => {
  expect(times(1.23e2, 2.45e2, 1.23e2)).toBe('3706605')
})
